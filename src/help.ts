...
const ACTION_EXAMPLES: Record<string, string> = {
	...
	search: '`codegraph({ action: "search", query: "useState", kind: "function" })`',
	explore: '`codegraph({ action: "explore", query: "auth session", max_files: 8 })`',
...
const action_example = (desc: ActionDescriptor): string => {
	return ACTION_EXAMPLES[desc.action] ?? `\`codegraph({ action: "${desc.action}" })\``
}
...
const supported_reference = (): string => {
	const entries = Object.values(DESCRIPTORS).sort((a, b) => a.action.localeCompare(b.action))
	...
	out += '\n'
	out += '> **Note:** `action: "search"` renders and runs as the CLI command `codegraph query`.\n'
...
const unsupported_reference = (): string => {
	const entries = Object.entries(UNSUPPORTED).sort(([a], [b]) => a.localeCompare(b))
