...
export default function register_codegraph_extension(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			action: Type.String({ description: 'CodeGraph action to execute (required)' }),
			query: Type.Optional(
				Type.String({ description: 'Natural-language or symbol/file search text' }),
			),
			symbol: Type.Optional(
				Type.String({ description: 'Symbol name for callers, callees, or impact' }),
			),
			file: Type.Optional(
				Type.String({ description: 'File path for node or symbol disambiguation' }),
			),
			...
			kind: Type.Optional(Type.String({ description: 'Symbol kind filter for search' })),
			max_files: Type.Optional(Type.Number({ description: 'Max files for explore' })),
			offset: Type.Optional(Type.Number({ description: 'Line offset for node slices' })),
			depth: Type.Optional(Type.Number({ description: 'Traversal depth for impact/affected' })),
			filter: Type.Optional(Type.String({ description: 'Filter expression for files/affected' })),
			format: Type.Optional(Type.String({ description: 'Output format for files' })),
			max_depth: Type.Optional(Type.Number({ description: 'Max directory depth for files' })),
			pattern: Type.Optional(Type.String({ description: 'Glob pattern filter for files' })),
			...
			files: Type.Optional(
				Type.Array(Type.String(), { description: 'Changed file paths for affected' }),
			),
			force: Type.Optional(
				Type.Boolean({ description: 'Force re-index when status reports old engine' }),
			),
		}),
