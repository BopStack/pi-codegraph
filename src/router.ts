...
const is_home_or_root = (path: string): boolean => {
	...
	return normalized === home || normalized === '/' || WINDOWS_ROOT_RE.test(normalized)
}
 ...
 */
const force_guard = (action: string, params: Record<string, unknown>): string | null => {
	if (params.force !== true) {
		return null
	}

	const project_path = typeof params.project_path === 'string' ? params.project_path : ''

	if (action === 'init' && (project_path.length === 0 || is_home_or_root(project_path))) {
		return (
	...
	if (action === 'index' && (project_path.length === 0 || is_home_or_root(project_path))) {
		return (
...
const execute_wired = async (
	...
	for (const name of descriptor.required) {
		if (!has_param(params, name)) {
			return {
				content: [{ type: 'text' as const, text: missing_param_text(action, name) }],
				details: {},
			}
		}
	}
	if (action === 'node' && !has_param(params, 'file') && !has_param(params, 'symbol')) {
		return {
			content: [
				{
					type: 'text' as const,
	...
	return format_output(runner_result, { action }) as AgentToolResult<Record<string, unknown>>
}
...
export const route_action = async (
	...
	if (action === 'help') {
		return { content: [{ type: 'text' as const, text: render_help() }], details: {} }
	}
	if (UNSUPPORTED[action]) {
		return { content: [{ type: 'text' as const, text: render_unsupported(action) }], details: {} }
	}
	...
	if (guard_text !== null) {
		return { content: [{ type: 'text' as const, text: guard_text }], details: {} }
	}
