...
const format_part = (part: string): string =>
...
export const render_result = (...all_args: unknown[]): Text => {
	...
	const context = is_pi_context
		? (arg3 as unknown as PiRenderContext)
		: undefined;
	...
	if (action.length > 0 && UNSUPPORTED[action] !== undefined) {
		text.setText(
			theme.fg(
				"warning",
				content_text || `CodeGraph action "${action}" is unsupported.`,
			),
		);
		return text;
	}
	...
	text.setText(
		`${theme.fg("toolOutput", first_line)}${theme.fg("dim", `  [${line_count} lines]`)}`,
	);
	return text;
};
