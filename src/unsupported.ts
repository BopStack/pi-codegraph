...
export const UNSUPPORTED: Record<string, UnsupportedEntry> = {
	...
	uninstall: {
		...
		safe_alternative: 'Run `codegraph uninstall` manually if you intend removal.',
	},
	daemon: {
		reason: 'Interactive daemon management is not available through the agent tool.',
	...
	install: {
		reason: 'Writes MCP configuration for other agents and may prompt interactively.',
		safe_alternative:
			'Run `codegraph install` or `codegraph install --print-config <target>` manually.',
	},
	install_config: {
		reason: 'Configures other agents rather than helping this agent understand code.',
		safe_alternative: 'Run `codegraph install --print-config <target>` manually.',
	},
	telemetry: {
		reason: 'Telemetry preferences and status are not code-understanding actions.',
		safe_alternative: 'Run `codegraph telemetry status/on/off` manually.',
	},
	upgrade: {
		reason: 'Package upgrade is a package-management concern, not code understanding.',
		safe_alternative: 'Run `codegraph upgrade` manually.',
	},
	upgrade_check: {
		reason: 'Upgrade checks are a package-management concern, not code understanding.',
		safe_alternative: 'Run `codegraph upgrade --check` manually.',
	},
	version: {
		reason: 'Standalone version check is an install diagnostic, not a code-understanding action.',
		safe_alternative: 'Check the installed version with `codegraph version` if needed.',
	},
}
